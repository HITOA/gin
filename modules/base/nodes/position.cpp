...
void Gin::Module::Base::Position::Execute(Graph::GraphContext ctx, Thread::ThreadPool& pool)
{
	SpatialOperation(pool, [this, ctx](size_t idx, size_t x, size_t y, size_t z) {
		position[idx] = Eigen::Vector3<double>{ (double)x,(double)y,(double)z } * ctx.scale - ctx.bounds.extent + ctx.bounds.origin;
	});
}
